...
from sql_schemas import Competitions, EventTypes, Matches, Events, Teams, PlayerPositions
...
def get_event(event_id: UUID, db: Session = Depends(get_db)):
    event = (db.query(
        ...
        .first())
    playerPositions = (
        db.query(PlayerPositions).filter(PlayerPositions.event_id == event_id).all()
    )
    if event is None:
